...
function installDouyinFudaiAutomation(options = {}) {
    ...
    const entranceUrl = options.entranceUrl || 'https://www.douyin.com/?recommend=1&from_nav=1';
    const config = {
        ...
        async runInLiveRoom(contentIndex) {
            ...
            while (Date.now() - startTime < config.luckyBagTotalTimeoutMs) {
                ...
                if (currentEntryNode) {
                    clickIndex += 1;
                    idleStartTime = 0;
                    setLogRemember(`第 ${contentIndex} 条内容第 ${clickIndex} 次点击左上角福袋入口`);
                    humanClick(currentEntryNode);
    ...
    async function ensureRecommendPage() {
        ...
        setLogRemember(`当前页面不是抖音推荐页：${location.href}，准备跳转到推荐页`);
        goToRecommendPage();
    ...
    function goToRecommendPage() {
        const recommendLink = document.querySelector('a[href*="recommend=1"][href*="from_nav=1"]');
        if (recommendLink) {
            recommendLink.click();
        ...
        setLogRemember('没有找到推荐入口，直接跳转抖音推荐页');
        location.href = entranceUrl;
        return false;
    }
